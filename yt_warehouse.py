...
with col1:
    st.header(':grey[Data collection in MongoDB]')
    st.write ('(tip: go to youtube channel > right click > view page source > find **?channel** the copy the ID)')
    channel_id = st.text_input('**Channel ID**')

    Get_data = st.button('**Collect in MonogoDB**')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        mydb = client['Youtube_lake']


        collection = mydb['Youtubelake_data']
...
with col2:
    st.header(':grey[Data Migrate Mongo to Mysql]')
    ...
    Migrate = st.button('**MongoDB to MySQL**')
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        mycursor.execute("CREATE DATABASE IF NOT EXISTS youtube_warehouse")
        ...
        engine = create_engine('mysql+mysqlconnector://root:rPWDmini_db@localhost/youtube_warehouse', echo=False)
...
st.header(':grey[Channel Data Analysis zone]')
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='rPWDmini_db', db='youtube_warehouse')
